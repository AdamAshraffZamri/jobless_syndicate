...
export default function MapContainer({ data, onStateClick }) {
  ...
  return (
    <div className="w-full h-full min-h-[500px] flex flex-col relative rounded-lg overflow-hidden bg-[#0B1120]">
      ...
      {/* Grid Pattern Background Container ("box box box") */}
      <div 
      ...
      >
        <ComposableMap projection="geoMercator" projectionConfig={{ scale: 2100 }} className="w-[90%] h-[90%] relative z-10">
                ...
                geographies.map((geo) => {
                  ...
                  return (
                    <Geography
                      ...
                      stroke="#0B1120"
                      strokeWidth={1}
                      onClick={() => { if (onStateClick && stateData && !isFilteredOut) onStateClick(stateData); }}
                      onMouseEnter={() => { if (stateData && !isFilteredOut) setHoveredState(stateData); }}
